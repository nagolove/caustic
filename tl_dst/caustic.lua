...
local parser_setup = {
   ...
   project = {
      options = { "-n --name" },
      summary = [[create project subdirectory with given name]],
   },
   ...
   publish = {
      summary = "собрать каталог проекта в ~/nagolove.github.io и запушить на web",
      options = { "-n --name" },
      flags = {
         { "-b --build", "пересобрать wasm-артефакты перед публикацией" },
         { "-y --yes", "не спрашивать подтверждения перед push" },
      },
   },
...
static CameraProcessor cam_proc;
...
int main(int argc, char **argv) {
    ...
    const char *wnd_name = "$artifact$";

    SetTraceLogLevel(LOG_WARNING);

#ifdef PLATFORM_WEB
    SetConfigFlags(FLAG_MSAA_4X_HINT);
    InitWindow(screen_width_web, screen_height_web, wnd_name);
#else
    ...
    koh_fpsmeter_init(GetScreenWidth());
    sc_init_script();
    koh_common_init();

    camp_init(&cam_proc, (CameraProcessorOpts) {
        .cam = &cam,
        .mouse_btn_move = MOUSE_BUTTON_RIGHT,
        .mod_key_down_scale = KEY_LEFT_SHIFT,
    });
   ...
   main_c = gsub(main_c, "%$artifact%$", artifact)
...
r
   ...
   ut.cmd_try(format("cp %s/gdb_remote.py .", e.path_caustic))
...
local function inject_link(lines, link)
   local before, section, after = {}, {}, {}
   local phase = 1
   ...
   if phase ~= 3 then
      printc("%{red}inject_link: секция ссылок не найдена%{reset}")
      return lines
   end

   local exists = false
   for _, l in ipairs(section) do
      if l == link then
         exists = true
         break
      end
   ...
   local out = {}
   for _, l in ipairs(before) do table.insert(out, l) end
   for _, l in ipairs(section) do table.insert(out, l) end
   for _, l in ipairs(after) do table.insert(out, l) end
   return out
end
...
local function sub_publish(_args, cfg)
   ...
   if not has_files_in_dir(files) then
      printc(
      "%{red}sub_publish: в каталоге проекта нет всех wasm-файлов " ..
      "(.data/.html/.js/.wasm). " ..
      "Соберите: koh make -t wasm  (или publish -b)%{reset}")
   ...
   local dist_dir = _args.name or string.match(src_dir, "%S+/(.*)")
   assert(dist_dir)
   ...
   if not ok then
      printc(
      "%{red}sub_publish: не удалось chdir('" .. site_repo_abs ..
      "'): " .. tostring(errmsg) .. "%{reset}")
   ...
   for _, file in ipairs(files) do
      cmd_do(format("cp %s/%s .", src_dir, file))
   end


   cmd_do(format("mv ./%s.html ./index.html", artifact))


   for _, file in ipairs(files) do
      local name = (file == artifact .. ".html") and "index.html" or file
      cmd_do(format("git add %s/%s", lfs.currentdir(), name))
   end
   ...
   local link = format(
   '<a href="/%s"><strong>%s</strong></a>', dist_dir, dist_dir)

   lines = inject_link(lines, link)
   ...
   cmd_do("git add index.html")


   printc(
   "%{green}publish: каталог '" .. dist_dir ..
   "' готов к публикации%{reset}")

   cmd_do("git --no-pager diff --cached --stat")

   if not _args.yes then
      local ans = readline("Закоммитить и запушить? (y/n) ")
      if ans ~= "y" and ans ~= "Y" then
         printc(
         "%{yellow}publish: отменено " ..
         "(изменения остались в индексе)%{reset}")

         return
      end
   end

   cmd_do(format('git commit -m "%s: обновление сайта"', dist_dir))
   cmd_do("git push origin master")
end
...
function actions.publish(_args)
   ...
   if not cfgs then
      printc(
      "%{red}actions.publish: нет bld.lua в текущем каталоге%{reset}")
   ...
   if _args.build then
      ...
      printc(
      "%{yellow}publish: пересборка не выполнялась " ..
      "(флаг -b/--build для сборки wasm); " ..
      "публикуются текущие артефакты%{reset}")
...
local function hex_hash32(s)
   ...
   for i = 1, #s do
      hash = (hash ~ string.byte(s, i)) & 0xffffffff
...
local function ninja_footer(f, c, ctx)
   assert(f)
   assert(c)
   assert(ctx)
...
function actions.cppcheck(_args)
   ...
   local excluded = {}
   if cfgs[1].cppcheck_excluded then
      for _, pat in ipairs(cfgs[1].cppcheck_excluded) do
         insert(excluded, pat)
      end
   end
   if _args.exclude then
      insert(excluded, _args.exclude)
   end
   ...
   if #excluded > 0 then
      ...
      for _, f in ipairs(files) do
         local skip = false
         for _, pat in ipairs(excluded) do
            if string.match(f, pat) then
               skip = true
               print(format(
               "cppcheck: исключён %s (шаблон '%s')", f, pat))
...
function actions.ctags(_args)
   local target = _args.t or "linux"
   ctags_write('tags.jsonl', target, _args)
end
...
local function main()
   ...
   if ok then
      ...
      local cmd = (_args).command

      if cmd and (actions)[cmd] then
         (actions)[cmd](_args)
      end
   else
