...
// Экземпляр навигатора (непрозрачный). Можно завести несколько независимых:
// у каждого свой индекс контролов, строка поиска и цель прыжка.
typedef struct ImNav ImNav;

// Создать/уничтожить экземпляр. Глобального экземпляра нет — владелец заводит
// свой и назначает активным (im_nav_active_set).
ImNav *im_nav_new(void);
void   im_nav_free(ImNav *nav);

// Активный экземпляр: именно ему макро-слой отдаёт marks (индексацию виджетов).
// Оберни рендер конкретной панели в active_set(nav)…active_set(prev), чтобы её
// контролы попали в отдельный индекс. Пока активный не назначен — marks
// игнорируются (можно снять индексацию, передав NULL).
void   im_nav_active_set(ImNav *nav);
ImNav *im_nav_active_get(void);

// Включить/выключить индексацию и прыжки в runtime (глобальный kill-switch,
// общий для всех экземпляров; по умолчанию включено).
void im_nav_set_enabled(bool enabled);
bool im_nav_is_enabled(void);

// Очистить накопленный индекс экземпляра (например, при смене сцены).
void im_nav_reset(ImNav *nav);

// Встраиваемый GUI поиска: рисует поле/кнопку/список ПРЯМО в текущее окно (без
// своего igBegin/igEnd) — можно вшить в чужую панель. Рисует переданный
// экземпляр. Тикает счётчик кадров и гасит просроченную цель.
void im_nav_gui(ImNav *nav);

// Тот же поиск в собственном окне «im_nav». Рисовать раз за кадр.
void im_nav_gui_window(ImNav *nav);
