...
// Работает быстро, возвращает кешированное значение. 
// Если компоненты данного типа еще не создавались, то будет выделена память.
...
/*
    То же, что e_get(), но доступ к компоненту по числовому cp_id, а не по
    объекту e_cp_type. Полезно, когда cp_id известен напрямую (сериализация,
    рефлексия, компонентный проводник).

    Возвращает указатель на данные компонента cp_id для сущности e либо NULL,
    если сущность невалидна, хранилище для cp_id не зарегистрировано, или у
    сущности нет этого компонента. В отличие от e_get(), хранилище НЕ создаётся.

    Внимание: использование невалидной сущности приводит к неопределённому
    поведению (проверяется entity_assert).
*/
