...
ObjectCouple objects_extract(cpArbiter *arb, int type_a, int type_b) {
    ObjectCouple oc = {0};
    ...
    Object *obj_a = a->userData, *obj_b = b->userData;

    if (!obj_a) {
        obj_a = shape_a->userData;
    }
    if (!obj_b) {
        obj_b = shape_b->userData;
    }

    if (obj_a && obj_a->type == type_a) {
        oc.a = obj_a;
    } else if (obj_b && obj_b->type == type_a) {
        oc.a = obj_b;
    }

    if (obj_a && obj_a->type == type_b) {
        oc.b = obj_a;
    } else if (obj_b && obj_b->type == type_b) {
        oc.b = obj_b;
    }
